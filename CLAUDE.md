...
- **Règle de répartition (identique cash / Mobile Money)** : prix = coût total de la course ; **livreur 88 %** (`montant_livreur`), **Sönaiyaa 12 %** (`commission_plateforme`). Économiquement c'est le livreur qui supporte la commission, mais elle est **garantie par le Crédit de l'expéditeur** (jamais réclamée au livreur → aucune dette).
- **Qui règle la course — `courses.payeur`** (migration `021`) :
  - `expediteur` : il remet la part livreur (88 %) — cash à la récupération ou Mobile Money (lien renvoyé dans `geniuspay_checkout_url`, pas envoyé au client). Le SMS client ne contient **aucun montant**.
  - `client` : **Mobile Money obligatoire** (client + cash → 400). Le client paie le prix complet ; au webhook `payment.success`, la commission est **rendue au Crédit** de l'expéditeur (coût 0 pour lui).
  - Défaut si l'app n'envoie pas `payeur` : `client` en Mobile Money, `expediteur` en cash. `Course.montant_a_encaisser` = montant réglé par le payeur (prix si client, part livreur si expéditeur).
- **Crédit (expéditeur)** : solde prépayé `expediteurs.credit_solde` + journal `credit_transactions` (migration `018`). Se recharge via PSP (`POST /expediteurs/me/credit/recharge` → webhook `payment.success` type `credit_recharge`). La **commission est débitée du Crédit à la création de toute course** (garde-fou : Crédit insuffisant → 400, rien créé), ajustée au partage GPS, rendue au paiement client ou à l'annulation via `credit_service.restituer_commission` (montant net calculé sous verrou → jamais rendue deux fois).
- **Gains (livreur)** : `livreur.solde_disponible` devient **positif uniquement** (courses Mobile Money + indemnités), retiré via `POST /livreurs/me/wallet/retrait`. **Plus de recharge ni de dette** côté livreur (course cash = part livreur remise en espèces par l'expéditeur, aucune écriture livreur).
